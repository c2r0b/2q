use indradb::{MemoryDatastore, Transaction, Identifier, Vertex};
...
async fn get_db(signal: broadcast::Sender<()>) -> Result<Database, String> {
    ...
    datastore.create_vertex(&vertex).map_err(|e| format!("Failed to create vertex: {:?}", e))?;

    // If no error, setup the Database struct (you may need to define it)
    let database = Database { graph: Arc::new(datastore) };
...
pub async fn init(client: Client, tx: mpsc::Sender<Payload>) {
    ...
    if let Err(e) = tx.send(Payload::new("IndraDB successfully connected".into())).await {
